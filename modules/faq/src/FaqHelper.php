...
class FaqHelper {
   ...
   *   The taxonomy term to build the breadcrumb for. Defaults to NULL.
   *
   * @return array
  ...
  public static function setFaqBreadcrumb($term = NULL) {
    ...
    if ($faq_settings->get('custom_breadcrumbs')) {
      if (\Drupal::moduleHandler()->moduleExists('taxonomy') && $term) {
        $breadcrumb[] = Link::fromTextAndUrl($term->getName(), Url::fromUserInput('/faq-page/' . $term->id()));
        $breadcrumb[] = Link::fromTextAndUrl($term->getName(), Url::fromUserInput('/faq-page/' . $term->id()));
      ...
      $breadcrumb[] = Link::fromTextAndUrl(t('Home'), Url::fromRoute('<front>')->setOptions(['attributes' => ['title' => $site_settings->get('name')]]));
  ...
  public static function taxonomyTermChildren($tid) {
    ...
    if (!isset($children)) {
      $result = \Drupal::database()->select('taxonomy_term__parent', 'tth')
        ->fields('tth', ['entity_id', 'parent_target_id'])
        ->execute();
   ...
   *   The theme hook used to render each child category.
   * @param int $default_weight
   *   The default weight used when a FAQ node has no explicit weight.
   * @param string $default_sorting
   *   The default sorting direction, either 'ASC' or 'DESC'.
   * @param string $category_display
   *   The layout of categories which should be used.
   * @param string $class
   *   The CSS class used to wrap each rendered category.
   * @param \Drupal\taxonomy\TermInterface|null $parent_term
   *   The parent term passed to the theme layer. Defaults to NULL.
   *
   * @return array
   *   The rendered output for each child category.
   */
  ...
  public static function viewChildCategoryHeaders($term) {
    ...
    foreach ($list as $child_term) {
      ...
      if ($term_node_count) {
        ...
        $term_vars['link'] = Link::fromTextAndUrl($child_term->getName(), Url::fromUserInput('/faq-page/' . $child_term_id));
        $term_vars['description'] = ($child_term->getDescription()) ? $child_term->getDescription() : '';
   ...
   *
   * @param string $path
   *   The path fragment to search for.
   *
   * @return string
   *   The index of the matching path fragment.
   */
